...
export class Cone {
    ...
    baseRadius;
    cap;
    axis; // unit, from apex to base
    k; // tan(theta) where theta is half-angle
    constructor(id, apex, axis, height, baseRadius, cap = "base") {
        this.id = id;
        this.apex = apex;
        this.height = height;
    ...
    intersect(ray, tMin, tMax) {
        ...
        if (this.cap === "base") {
            ...
            if (capHit && (!best || capHit.t < best.t))
                best = capHit;
        }
        return best;
    }
    hitBaseCap(ray, center, normal, tMin, tMax) {
        ...
        if (t <= EPS || t < tMin || t > tMax)
            return null;
        const p = Vec3.add(ray.origin, Vec3.mulScalar(ray.dir, t));
        if (Vec3.distanceSq(p, center) > this.baseRadius * this.baseRadius + 1e-8)
            return null;
