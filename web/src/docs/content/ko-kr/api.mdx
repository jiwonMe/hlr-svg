...
| `mesh`       | `object`  | -       | triangle mesh feature-line 옵션 |
...
| `strokeVisible`      | `string` | `"#000"` | 보이는 선 색상      |
| `strokeWidthHidden`  | `number` | `1.5`    | 숨겨진 선 두께      |
| `strokeHidden`       | `string` | `"#000"` | 숨겨진 선 색상      |
...
| `meshEdges`     | `boolean` | `true` | `TriangleMesh`의 boundary / crease / silhouette edge |
...
| 속성             | 타입     | 기본값 | 설명 |
| ---------------- | -------- | ------ | ---- |
| `creaseAngleDeg` | `number` | `30`   | 인접 triangle face를 feature edge로 승격시키는 각도 임계값 |
...
OBJ/STL에서 가져온 geometry를 담는 triangle mesh primitive입니다.
...
| ----------- | --------------------------------- | ---- |
| `id`        | `string`                          | 고유 식별자 |
| `vertices`  | `Vec3[]`                          | 메시 정점 위치 |
| `triangles` | `Array<[number, number, number]>` | triangle 인덱스 목록 |
...
- `mtllib`, `usemtl`, `vt`, `l`, `p`, `curv`는 warning과 함께 무시됩니다.
- mesh도 HLR raycasting에는 참여하지만, analytic intersection curve는 기존 analytic primitive에만 적용됩니다.
