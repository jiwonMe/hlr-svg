...
| `mesh`       | `object`  | -        | Triangle mesh feature-line options |
...
| `strokeVisible`      | `string` | `"#000"` | Visible line color       |
| `strokeWidthHidden`  | `number` | `1.5`    | Hidden line width        |
| `strokeHidden`       | `string` | `"#000"` | Hidden line color        |
...
| `meshEdges`     | `boolean` | `true`  | `TriangleMesh` boundary / crease / silhouette edges |
...
| Property         | Type     | Default | Description |
| ---------------- | -------- | ------- | ----------- |
| `creaseAngleDeg` | `number` | `30`    | Angle threshold for turning adjacent triangle faces into feature edges |
...
Triangle mesh primitive for imported OBJ/STL geometry.

```typescript
import { TriangleMesh, Vec3 } from "hlr";
...
| ----------- | --------------------------------- | ----------- |
| `id`        | `string`                          | Unique identifier |
| `vertices`  | `Vec3[]`                          | Mesh vertex positions |
| `triangles` | `Array<[number, number, number]>` | Triangle index list |

### OBJ / STL Import

```typescript
import { parseObj, parseStl } from "hlr";

const obj = parseObj(
  await fetch("/model.obj").then((res) => res.arrayBuffer()),
);
const stl = parseStl(
  await fetch("/model.stl").then((res) => res.arrayBuffer()),
);
```
...
- Meshes participate in HLR raycasting, but analytic intersection curves are still limited to the built-in analytic primitives.
...
const svg = renderer.render(scene, camera, {
  hlr: {
    samples: 192, // Sampling count (↑ = precise, slow)
...
const customLine = lineToCubic3(new Vec3(0, 0, 0), new Vec3(1, 1, 1));
