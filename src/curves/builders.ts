...
import type { CubicBezier3 } from "./cubicBezier3.js";

export function lineToCubic3(p0: Vec3, p3: Vec3): CubicBezier3 {
...
export function cylinderSilhouetteToCubics3(opts: {
  ...
  if (wLen <= opts.radius * (1 + 1e-8)) return [];
...
export function coneSilhouetteToCubics3(opts: {
  ...
  if (m <= 1e-8) return []; // camera on axis -> degenerate, base rim is already drawn

  const rhs = (k * qa) / m;
  if (Math.abs(rhs) > 1) return []; // no real tangents (shouldn't happen for external camera)
...
function orthonormalBasisFromNormal(n: Vec3): { u: Vec3; v: Vec3 } {
...
function arc90ToCubic3(center: Vec3, u: Vec3, v: Vec3, r: number, a0: number, a1: number): CubicBezier3 {
...
function pointOnCircle(center: Vec3, u: Vec3, v: Vec3, r: number, c: number, s: number): Vec3 {
