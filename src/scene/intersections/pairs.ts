...
function splitRunsByKeyJump(
  points: Vec3[],
  keys: number[],
  maxKeyJump: number,
): Vec3[][] {
  if (points.length < 2 || keys.length !== points.length)
...
function makeRunsForBranch(opts: {
  ...
  const keyRuns = baseRuns.flatMap((r) =>
    splitRunsByKeyJump(r, sliceKeysForRun(branch, keys, r), keyJump),
  );
  ...
  return stitchRunsByEndpointsWithDirection(
    mergeCyclicRuns(keyRuns, closeEpsSq),
    closeEpsSq,
    { minTangentCos: 0.25 },
  );
}
...
export function intersectSphereCylinder(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const w = Vec3.add(
      Vec3.mulScalar(u, Math.cos(th)),
      Vec3.mulScalar(v, Math.sin(th)),
    );
    ...
    const roots = solveQuadratic(1, b, cc).filter(
      (h) => h >= 0 && h <= c.height,
    );
    const candidates: KeyedCandidate3[] = roots.map((h) => ({
      key: h,
      point: Vec3.add(
        Vec3.add(base, Vec3.mulScalar(a, h)),
        Vec3.mulScalar(w, c.radius),
      ),
    }));
  ...
  if (!useBezierFit)
    return [
      ...polylineRunsToLineCubics(runs0, closeEpsSq),
      ...polylineRunsToLineCubics(runs1, closeEpsSq),
    ];
  if (fitMode === "stitchThenFit") {
    ...
    return [
      ...(m0
        ? fitPolylineToCubics(m0, { maxError, closeEps: step * 3 })
        : runs0.flatMap((run) =>
            fitPolylineToCubics(run, { maxError, closeEps: step * 3 }),
          )),
      ...(m1
        ? fitPolylineToCubics(m1, { maxError, closeEps: step * 3 })
        : runs1.flatMap((run) =>
            fitPolylineToCubics(run, { maxError, closeEps: step * 3 }),
          )),
    ];
...
export function intersectSphereCone(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const roots = solveQuadratic(A, B, C).filter(
      (y) => y >= 0 && y <= cone.height,
    );
    const candidates: KeyedCandidate3[] = roots.map((y) => ({
      key: y,
      point: Vec3.add(
        Vec3.add(cone.apex, Vec3.mulScalar(a, y)),
        Vec3.mulScalar(w, k * y),
      ),
    }));
    assignBranchesKeyed(branch0, branch1, keys0, keys1, candidates);
  }
...
export function intersectCylinderCylinder(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const y0 = Vec3.add(
      Vec3.sub(c0.base, c1.base),
      Vec3.mulScalar(w, c0.radius),
    );

    const roots = solveQuadraticCylinderConstraint(y0, a0, c1).filter(
      (s) => s >= 0 && s <= c0.height,
    );
...
export function intersectCylinderCone(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const roots = solveQuadraticConeConstraint(x0, a, cone).filter(
      (s) => s >= 0 && s <= cyl.height,
    );
    const candidates: KeyedCandidate3[] = roots
      .map((sVal) => ({
        key: sVal,
        point: Vec3.add(x0, Vec3.mulScalar(a, sVal)),
      }))
...
export function intersectConeCone(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const roots = solveQuadraticConeRayConstraint(cone0.apex, d, cone1).filter(
      (y) => y >= 0 && y <= cone0.height,
    );
    const candidates: KeyedCandidate3[] = roots
      .map((yVal) => ({
        key: yVal,
        point: Vec3.add(cone0.apex, Vec3.mulScalar(d, yVal)),
      }))
  ...
  const step =
    (2 * Math.PI * Math.max(1e-3, cone0.baseRadius, cone1.baseRadius)) / n;
  ...
  const runs0 = stitchRunsByEndpointsWithDirection(
    mergeCyclicRuns(
      baseRuns0.flatMap((r) =>
        splitRunsByKeyJump(r, sliceKeysForRun(branch0, keys0, r), keyJump0),
      ),
      closeEpsSq,
    ),
    closeEpsSq,
  ...
  const runs1 = stitchRunsByEndpointsWithDirection(
    mergeCyclicRuns(
      baseRuns1.flatMap((r) =>
        splitRunsByKeyJump(r, sliceKeysForRun(branch1, keys1, r), keyJump1),
      ),
      closeEpsSq,
    ),
    closeEpsSq,
  ...
  if (fitMode === "stitchThenFit") {
    ...
    return [
      ...(m0
        ? fitPolylineToCubics(m0, {
            maxError,
            closeEps: step * 3,
            maxAlphaFactor: 1.2,
          })
        : runs0.flatMap((run) =>
            fitPolylineToCubics(run, {
              maxError,
              closeEps: step * 3,
              maxAlphaFactor: 1.2,
            }),
...
function pointOnCylinder(c: Cylinder, wUnit: Vec3, s: number): Vec3 {
  return Vec3.add(
    Vec3.add(c.base, Vec3.mulScalar(c.axis, s)),
    Vec3.mulScalar(wUnit, c.radius),
  );
}
...
function solveQuadraticCylinderConstraint(
  y0: Vec3,
  d: Vec3,
  c: Cylinder,
): number[] {
...
function solveQuadraticConeRayConstraint(
  origin: Vec3,
  d: Vec3,
  cone: Cone,
): number[] {
